...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            raise ValueError(f"参数值错误：{transition}")
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for i in range(len(partial_parses)):
            if dependencies[i] is None and len(partial_parses[i].stack)==1 and len(partial_parses[i].buffer)==0:
                dependencies[i] = partial_parses[i].dependencies.copy()
            elif dependencies[i] is None:
                unfinished_parses.append(partial_parses[i])
