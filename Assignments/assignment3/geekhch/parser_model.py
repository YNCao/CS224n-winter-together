...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        embed_to_hidden_weight = nn.Parameter(torch.empty((self.embed_size * n_features, self.hidden_size)), True)
        self.embed_to_hidden_weight = nn.init.xavier_uniform_(embed_to_hidden_weight)
        embed_to_hidden_bias = nn.Parameter(torch.empty(hidden_size, ), True)
        self.embed_to_hidden_bias = nn.init.uniform_(embed_to_hidden_bias)

        # dropout = torch.rand(hidden_size).apply_(lambda x: 0 if x<dropout_prob else 1).view((hidden_size, 1))
        # self.dropout = dropout * (hidden_size / dropout.sum().item())
        self.dropout = nn.Dropout(dropout_prob)

        hidden_to_logits_weight = nn.Parameter(torch.empty(hidden_size, n_classes), True)
        self.hidden_to_logits_weight = nn.init.xavier_uniform_(hidden_to_logits_weight)

        hidden_to_logits_bias = nn.Parameter(torch.empty(n_classes), True)
